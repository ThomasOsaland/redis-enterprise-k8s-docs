...
| dbIngressSuffix | DB ENDPOINT SUFFIX - will used to set the db host ingress <db name><db ingress suffix> Creates a host name so it should be unique if more than one db is created on the cluster with the same name | string |  | true |
...
| dbIngressSuffix | DB host SUFFIX - will used to set the db host ingress: <db name><db ingress suffix> Creates a host name so it should be unique if more than one db is created on the cluster with the same name | string |  | true |
...
| redisEnterpriseNodeResources | Compute resource requirements for Redis Enterprise pods | *[v1.ResourceRequirements](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.11/#resourcerequirements-v1-core) | 2 CPUs and 4GB memory | false |
